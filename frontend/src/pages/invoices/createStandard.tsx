...
const sectionStyle = { borderRadius: 8, padding: 24, marginBottom: 24, boxShadow: '0 1px 4px #eee' };
...
export const StandardInvoiceForm: React.FC = () => {
  ...
  const invoiceType = location.state?.invoiceType || "standard";
  ...
  return (
    <Card title={<Typography.Title level={3}>📄 Standard Invoice (B2B)</Typography.Title>} bordered style={{ maxWidth: 1100, margin: "32px auto" }}>
      <Form
        layout="vertical"
        form={form}
        onFinish={(v) => {
      ...
      >
        <Form.Item label="Invoice Type" name="invoiceType" initialValue={invoiceType} rules={[{ required: true }]}> <Input value={invoiceType} disabled /> </Form.Item>
        {/* Seller Section */}
        ...
        <div style={sectionStyle}>
          ...
          {lineItems.map((item, idx) => (
            <Row gutter={16} key={idx} align="middle" style={{ marginBottom: 8 }}>
              ...
              <Col span={4}>
                <InputNumber min={1} value={item.quantity} onChange={v => handleLineItemChange(idx, "quantity", v)} style={{ width: "100%" }} placeholder="Quantity" />
              </Col>
              <Col span={5}>
                <InputNumber min={0} step={0.01} value={item.unitPrice} onChange={v => handleLineItemChange(idx, "unitPrice", v)} style={{ width: "100%" }} placeholder="Unit Price (excl. VAT)" />
              </Col>
              <Col span={4}>
                <InputNumber min={0} max={15} step={0.5} value={item.vatPercent} onChange={v => handleLineItemChange(idx, "vatPercent", v)} style={{ width: "100%" }} placeholder="VAT %" />
              </Col>
              <Col span={2}>
        ...
        {/* Invoice Status Section - NEW SECTION ADDED */}
        <div style={sectionStyle}>
          <div style={sectionTitleStyle}>Invoice Status</div>
          <Row gutter={24}>
            <Col span={24}><Form.Item label="Invoice Status" name="status" rules={[{ required: true }]}> <Select options={[{ value: "Draft", label: "Draft" }, { value: "Validation Pass", label: "Validation Pass" }, { value: "Submitted to ZATCA", label: "Submitted to ZATCA" }]} placeholder="Select status" /> </Form.Item></Col>
          </Row>
        </div>
